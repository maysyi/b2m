...
import pyaudio
import numpy as np
from music import scale_note, get_octave, get_note, off_note
import threading
import time
import mido
...
def play_audio():
    ...
    while running:
        ...
        if stable and current_button == 1 and not pressed:
            ...
            msg_on = get_note(note_name, midi_dict)
            outport.send(msg_on)
            ...
            if note_name != current_note_name:
                midi_dict = get_octave(current_octave)
                # turn off previous
                msg_off = off_note(current_note_name, midi_dict)
            ...
            msg_off = off_note(note_name, midi_dict)
            outport.send(msg_off)
            pressed = False
        time.sleep(0.003)
